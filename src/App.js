...
const ProjectStatusDashboard = () => {
  ...
  async function doIgnore() {
    try {
      ...
      // Toggle ignore status instead of just setting to true
      const updatedComment = {
        id: commentId,
        text: comment.text, // Preserve the original text
        ignored: !comment.ignored, // Toggle ignore status
        deleted: false,
        ts: new Date().toISOString()
      };
      ...
      await addHistoryEntry(projectId, `${new Date().toLocaleString()}: Comment ${action}${isAdmin ? ' [Admin]' : ''}`);

      setState(prev => ({
        ...prev,
        projects: prev.projects.map(p => {
          if (p.id !== projectId) return p;
          const newComments = p.comments.map(c => c.id === commentId ? ({ ...c, ignored: !c.ignored, deleted: false, ts: new Date().toISOString() }) : c);
          return { ...p, comments: newComments, history: [...(p.history || []), `${new Date().toLocaleString()}: Comment ${action}${isAdmin ? ' [Admin]' : ''}`] };
        })
      }));
  ...
  return (
    <div style={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'center', textAlign: 'center' }}>
                      ...
                      <select
                        ...
                        style={{
                          ...
                          borderRadius: '8px',
                          border: '1px solid var(--gray-3)',
                          width: '140px', // Same width as status
                          height: '40px', // Same height as status
                          textAlign: 'center',
                          fontWeight: 600,
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        ...
                        <button 
                          ...
                          style={{
                            background: c.ignored ? 'var(--success)' : 'var(--warning)',
                            color: 'white',
                            border: 'none',
                            padding: '6px 10px',
                            borderRadius: '8px',
                        ...
                        >{c.ignored ? 'Unignore' : 'Ignore'}</button>
                      </div>
                    </div>
                    {editingId === c.id && (
      ...
      {/* Add Project Modal - ТОЛЬКО ДЛЯ АДМИНА */}
      ...
      {confirmIgnore && (
        <div className="modal-backdrop" style={{
          ...
          <div className="modal-content" style={{
            ...
            <div style={{ fontWeight: 700, marginBottom: '8px' }}>{state.projects.find(p => p.id === confirmIgnore.projectId)?.comments?.find(c => c.id === confirmIgnore.commentId)?.ignored ? 'Unignore' : 'Ignore'} comment?</div>
            <div style={{ marginBottom: '16px' }}>Are you sure you want to {state.projects.find(p => p.id === confirmIgnore.projectId)?.comments?.find(c => c.id === confirmIgnore.commentId)?.ignored ? 'unignore' : 'ignore'} this comment?</div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              ...
              <button onClick={doIgnore} style={{
                background: state.projects.find(p => p.id === confirmIgnore.projectId)?.comments?.find(c => c.id === confirmIgnore.commentId)?.ignored ? 'var(--success)' : 'var(--warning)',
                color: 'white',
                border: 'none',
                padding: '8px 16px',
                borderRadius: '10px',
